...
/*   Updated: 2021/05/16 18:30:36 by srakuma          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*ft_get_int(t_format *f_spec, va_list ap)
{
	char	*str;

	if (f_spec->specifier[3] || f_spec->specifier[4])
		str = ft_itoa((int)va_arg(ap, int));
	else if (f_spec->specifier[5])
		str = ft_utoa((unsigned int)va_arg(ap, unsigned int));
	else if (f_spec->specifier[2])
		str = ft_xtoa((unsigned long int)va_arg(ap, long int), "abcdef");
	else if (f_spec->specifier[6])
		str = ft_xtoa((unsigned long int)va_arg(ap, unsigned int), "abcdef");
	else
		str = ft_xtoa((unsigned long int)va_arg(ap, unsigned int), "ABCDEF");
	if (*str == '0' && f_spec->prcn == 0)
	{
		f_spec->flag[4] = 0;
		if (f_spec->specifier[2])
			return (ft_strdup("0x"));
